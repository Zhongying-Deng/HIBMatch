...
def plot_embedding(X, y, d, fig_mode='save', title=None, imgName=None, plot_tar_only=False):
    """
    ...
    for i in range(X.shape[0]):
        # plot colored number which represents the class label
        if plot_tar_only:
            # if ploting only one domain, then the colors denote class label
            ...
            # if ploting two domains, then the colors denote domain label
            plt.text(X[i, 0], X[i, 1], str(y[i]),
                 #color=plt.cm.bwr(d[i] / 1.),
                 color=plt.cm.rainbow(d[i] / 1.),
                 #color=plt.cm.rainbow(y[i] / y.max()),
...
def tsne(feat_source, feat_target, s_labels, t_labels, imgName, domain_x=None, plot_tar_only=True, save=True, title=None,
    ...
    if save:
        ...
        fig_mode = 'display'
    if torch.is_tensor(feat_source):
        feat_source = feat_source.detach().numpy()
    if torch.is_tensor(feat_target):
        feat_target = feat_target.detach().numpy()
    if torch.is_tensor(t_labels):
        t_labels = t_labels.detach().numpy()

    if not plot_tar_only:
        dann_tsne = tsne.fit_transform(np.concatenate((feat_source, feat_target)))
        plot_embedding(dann_tsne, np.concatenate((s_labels, t_labels)),
                np.concatenate((s_domain_tags, t_domain_tags)), fig_mode, title=title, imgName=imgName, plot_tar_only=plot_tar_only)
    else:
        dann_tsne = tsne.fit_transform(feat_target)
        plot_embedding(dann_tsne, t_labels, t_domain_tags.detach().numpy(), fig_mode, title=title, imgName=imgName, plot_tar_only=plot_tar_only)
