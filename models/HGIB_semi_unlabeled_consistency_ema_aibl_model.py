...
class HGIBSemiUnlabeledConsistencyEMAAIBLModel(BaseModel):
    def name(self):
        return 'HGIBSemiUnlabeledConsistencyEMAAIBLModel'
    ...
    def initialize(self, opt):
        ...
        self.beta = opt.beta
        self.weight_focal = opt.weight_focal
        self.weight_constrast = opt.weight_constrast
        self.best_acc = 0.
        self.fold = opt.fold
        self.epoch = 0
        self.visualize_save_path = 'checkpoints/{}'.format(opt.name, opt.fold)
        ...
        self.model_names = ['Encoder_MRI', 'Encoder_NonImage_New', 'Decoder_HGIB']
        ...
        self.netEncoder_NonImage_New = networks3D.init_net_update(networks3D.Encoder_NonImage(in_channels=3, out_channels=1024, dropout_prob=0.5), self.gpu_ids)
        
        self.num_graph_update = opt.num_graph_update
        self.weight_u = opt.weight_u # loss weight for unlabeled data
        self.netClassifier = torch.nn.Linear(1024*2, self.class_num)
        if len(self.gpu_ids) > 0:
            assert(torch.cuda.is_available())
            self.netClassifier.to(self.gpu_ids[0])
            self.netClassifier = torch.nn.DataParallel(self.netClassifier, self.gpu_ids)
        self.netDecoder_HGIB = networks3D.init_net_update(HGIB_v1(1024*2, 1024, self.class_num, use_bn=False, heads=1), self.gpu_ids)
        if self.class_num == 1:
        ...
        if self.use_ema:
            ...
            self.ema_model_non_img = ema.ModelEMA(self.gpu_ids[0], self.netEncoder_NonImage_New, 0.999)
        ...
        if self.isTrain:
            self.optimizer = torch.optim.AdamW([{'params': self.netDecoder_HGIB.parameters()}, 
                                                ...
                                                {'params': self.netEncoder_NonImage_New.parameters()},
    ...
    def set_input(self, input):
        ...
        self.nonimage = input[2].to(self.device)
        ...
        if phase == 'test':
            with torch.no_grad():
                if self.use_ema:
                    ...
                    self.embedding_NonImage = self.netEncoder_NonImage_New(self.nonimage)
        else:
            self.embedding_MRI = self.netEncoder_MRI(self.MRI)
            self.embedding_NonImage = self.netEncoder_NonImage_New(self.nonimage)
        return self.embedding_MRI, self.embedding_NonImage

    def HGconstruct(self, embedding_MRI, embedding_NonImage):
        ...
        self.embedding = torch.Tensor(np.hstack((embedding_MRI, embedding_NonImage))).to(self.device)
    ...
    def forward(self, phase='train', train_loader=None, test_loader=None, train_loader_u=None, epoch=None):
        ...
        if phase == 'train':
            ...
            for i in range(len_train_loader): 
                ...
                embedding = torch.cat((self.embedding_MRI, self.embedding_NonImage), dim=1)
                if self.robust_noise:
                    embedding = self.add_noise(embedding, lam=0.1)
                ...
                # self.G = self.G.drop_hyperedges(drop_rate=0.6)
                if self.robust_noise:
                    self.embedding = self.add_noise(self.embedding, lam=0.1)
                ...
                if self.use_ema:
                    ...
                    self.ema_model_non_img.update(self.netEncoder_NonImage_New)
            ...
            MRI, Non_Img, Label, length = self.get_features([train_loader, test_loader])
            # create hypergraph
            self.HGconstruct(MRI, Non_Img)
            self.info(length)
            ...
            self.epoch += 1
            with torch.no_grad():
                MRI, Non_Img, Label, length = self.get_features([train_loader, test_loader], phase=phase)
                # create hypergraph
                self.HGconstruct(MRI, Non_Img)
                self.info(length)
                self.set_HGinput(Label)
    ...
    def add_noise(self, embedding, lam=0.01):
        # Robustness test
    ...
    def get_features(self, loaders, phase: str='test'):
        ...
        for idx, loader in enumerate(loaders):
            for i, data in enumerate(tqdm(loader)):
                self.set_input(data)
                i_MRI, i_Non_Img = self.ExtractFeatures(phase)
                if MRI is None:
                    MRI = i_MRI
                    Non_Img = i_Non_Img
                    Label = data[1]
                    ...
                    Label = torch.cat([Label, data[1]], 0)
            length[idx] = MRI.size(0)
        length[1] = length[1] - length[0]
        return MRI.cpu().detach().numpy(), Non_Img.cpu().detach().numpy(), Label, length
