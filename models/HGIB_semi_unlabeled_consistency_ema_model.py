        ...
        if phase == 'train':
            ...
            for i in range(len_train_loader): 
                ...
                if self.robust_noise:
                    embedding = self.add_noise(embedding, lam=0.05)
                ...
                # self.G = self.G.drop_hyperedges(drop_rate=0.4)
                if self.robust_noise:
                    self.embedding = self.add_noise(self.embedding, lam=0.05)
                ...
                loss_u = (self.contrastive_loss(embedding_MRI, embedding_MRI_str_aug) + self.contrastive_loss(embedding_PET, embedding_PET_str_aug)) + \
                    self.weight_constrast * (self.contrastive_loss(embedding_MRI, embedding_PET_str_aug) + self.contrastive_loss(embedding_PET, embedding_MRI_str_aug))
